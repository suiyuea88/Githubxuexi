...
> V1.1.0 分类增强版：黛青国风书院 UI、接口强连接、GitHub 全站搜索与十类项目入口、临时浏览器运行、运行前仓库检测、README 小白导学、英文翻译、报错诊断、学习笔记和个人学习中心。
...
开始导入前会调用运行检测接口，核对仓库根目录的 `package.json`、`index.html`、前端框架依赖和 `dev/start/serve/preview` 脚本。未通过检测的 Skill、CLI、素材库或后端项目不再进入 StackBlitz 无限加载；已通过的项目如果导入超过 35 秒，页面会提供新窗口运行和 README 导学备用选项。

“小白开源翻译器”支持英文 README、报错信息和安装说明，单次最多 6000 字符；反引号和代码块中的命令会尽量保持原样。翻译内容不落库，关闭即清除。

个人学习中心会在浏览器本地记录已看项目、README 练习、运行步骤、收藏和笔记，并生成“读懂项目 → 看懂 README → 成功运行 → 解决报错 → 完成小修改”学习路线。不需要数据库，不增加 Render 保存成本。

项目藏馆支持 AI、Python、Web、开发工具、自动化、机器人、数据科学、移动应用、网络安全和游戏开发十个方向。点击分类会查询整个 GitHub，每次展示 20 个热门项目并可继续加载，不再只从首页二十个项目中筛选。搜索框也支持项目名、作者、语言、标签及 GitHub 高级语法；“TG机器人”等常用中文词会自动转换为 GitHub 更容易识别的英文关键词。

## 目录结构

- `backend/`：FastAPI 接口、GitHub 数据抓取与中文分析
- `frontend/`：静态网页首页、项目详情页与国风背景
- `部署说明.md`：Render 填写方法、上线检查与常见问题
- `README.md`：项目说明
